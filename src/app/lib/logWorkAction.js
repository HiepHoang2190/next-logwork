...
export const logTimeTotalIssue = (arrLog = []) => {
  ...
  const final =
    ...
    filteredArr
      .map((item) =>
        Object.values(item["logs"]).reduce(
          (a, b) => Number(a) + Number((b["timeworked"] / 3600).toFixed(2)),
          0
        )
      )
  ...
  filteredArr &&
    filteredArr.forEach((item) => {
      Object.values(item["logs"]).forEach((item2) => {
        const createDate =
          item2["created"] && item2["created"].substring(0, 10);
...
export const filterWorklogsByAuthor = async (data, authorName, month, year) => {
  const newData = await Promise.all(
    data.map(async (item) => {
      ...
      const filterWorklogsByMonth = filterWorklogs(
        filteredWorklogs,
        month,
        year
      );
